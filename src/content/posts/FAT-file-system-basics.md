...
![descriptor](./_assets/fat_fs/descriptor.png)

Each disk has 2 FAT's (one is for backup). But A disk may also have directories. One directory known as the _root directory_, is present on every disk, but the root may habe multiple _sub-directories_, nested one inside of each other to form a tree structure. These sub-directories can be used, created, removed and modified by the user. Therefore the structure can be as simple and as complex as the user makes it.
